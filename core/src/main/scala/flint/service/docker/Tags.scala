...
object Tags {
  ...
  def apply(
    ...
    val authToken: ValidationNel[String, String] = fromJSON[TokenResponse](tokenJSON) match {
      ...
      case Failure(errors) =>
        Failure(s"Failed to extract token from dockerhub server response JSON: ${tokenJSON}"
    ...
    authToken.flatMap { authToken =>
      ...
      fromJSON[TagResponse](tagJSON) match {
        ...
        case Failure(errors) =>
          Failure(s"Failed to extract tags from dockerhub server response JSON: ${tagJSON}"
            <:: errors.map(_.toString))
      }
    }
  }
